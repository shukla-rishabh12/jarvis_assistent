...
def send_whatsapp_via_gemini(user_command):
    ...
    try:
        ...
        cleaned = re.sub(r"```.*?```", "", ask_gemini(prompt), flags=re.DOTALL).strip()
        # gemini_response = 
        # print("🧠 Gemini response (raw):", gemini_response)

        # 2️⃣ Clean response: remove ```json``` or ``` backticks
        ...
        # 3️⃣ Check contact
        if contact_name not in [c.lower() for c in contacts]:
