...
# Add near top of intent checks (before fallback)
    ...
    if any(word in command for word in ["shutdown", "shut down", "power off", "restart", "reboot", "hibernate", "हाइबरनेट", "बंद करो", "शटडाउन", "रीस्टार्ट"]):
        prompt = f"User said: {cmd}\nReturn only system action: shutdown, restart, sleep, or lock."
        gemini_command = ask_gemini(prompt).strip().lower()
        result = handle_shutdown_command_from_gemini(command)

    # result is a string describing action
